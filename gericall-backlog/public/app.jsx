...
function Backlog() {
  ...
  /* load */
  useEffect(() => {
    fetch("/api/backlog").then((r) => r.json()).then(setItems);
  }, []);
  ...
  const scheduleSave = useCallback(() => {
    ...
    saveTimer.current = setTimeout(async () => {
      await fetch("/api/backlog", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ items: itemsRef.current }) });
      setToast(true);
